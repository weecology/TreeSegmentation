...
#path_to_tiles="/orange/ewhite/b.weinstein/NEON/D03/OSBS/L1/DiscreteLidar/Cropped/2017/cropped_"
...
```{r}
ptlist<-list(dalponte=dalponte$tile,li=li$tile,watershed=watershed_result$tile,silva=silva$tile)
...
```{r,eval=F}
results<-evaluate(ground_truth=itcs[[8]],algorithm = c("silva","dalponte","li"),path_to_tiles=path_to_tiles,compute_consensus = F,plot_results=T)
...
```{r,eval=T}
system.time(results_all<-evaluate_all(itcs=itcs,algorithm = c("dalponte","li","watershed"),path_to_tiles="/Users/ben/Dropbox/Weecology/NEON/cropped_",cores=cores,extra=F,compute_consensus=F))
